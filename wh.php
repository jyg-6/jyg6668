...
class DatabaseTool{
	...
	public function backup($tables=array()){
		...
		if(!empty($this->tables)){
			...
			$this->writeToFile($this->tables,$ddl,$data);#开始写入
		}else{
			$this->error='数据库中没有表!';
			return false;
		}
	...
	private function setTables($tables=array()){
		if(!empty($tables)){
			$this->tables=$this->getTables();#备份全部表
		}else{
			$this->tables=$tables;#备份指定表
		}
	}
	...
	private function parseSQL($path=''){
		...
		$sql=array_filter($sql,'del');#先消除--注释
		$sql=implode('',$sql);
		$sql=preg_replace('/\/\*.*\*\//','',$sql);#删除/**/注释
		return $sql;
	}
}
...
function login($user,$pass){
	...
	if($user!=$username || $pass!=$password){
		...
		if($block[0]<4 || (time()-$block[1])>$blocktime){
			if(!empty($user) || !empty($pass))$note='<font class=error>**错误：用户名或密码错误！**</font><br><br>';
			echo $note.'<form action="'.SCRIPT_NAME.'" method="post"><table align="center"><tr><td><font size="2">用户名: </font><td><input type="text" name="user" size="18" border="0" class="text" value="'.$user.'" style="width:150px"><tr><td><font size="2">密码: </font><td><input type="password" name="pass" size="18" border="0" class="text" value="'.$pass.'" style="width:150px"><tr><td colspan="2"><input type="submit" value="登录" border="0" class="button"></table></form>';
		}else{
			echo '错误尝试太多，服务器已处于锁定状态，请稍侯重新打开页面再试！';
		}
		mainbottom();
		exit;
	}else{
...
function logout(){
	...
	echo '你已成功退出!<br><br>点击这里<a href="'.SCRIPT_NAME.'">重新登录</a>。';
}

function maintop($title){
	$sitetitle='LOTUS在线文件管理系统';
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			...
			function Ask(note){var item=['删除'];if(item.indexOf(note)>-1){if(!confirm("您确定要执行"+note+"操作吗？"))return false;}else return true;}
	...
	if($title!='登录'){
		echo <<<OUTPUT
				<div id="menutable">
					<table id="menu">
						<tr>
							<td><a href="$script_name">文件管理</a></td>
							<td><a href="$script_name?action=upload">上传文件</a></td>
							<td><a href="$script_name?action=teldown">远程下载</a></td>
							<td><a href="$script_name?action=sitezip">全站备份</a></td>
							<td><a href="$script_name?action=backup">数据库备份</a></td>
							<td><a href="$script_name?action=store">数据库恢复</a></td>
							<td><a href="$script_name?action=ftplogin">远程FTP功能</a></td>
							<td><a href="$script_name?action=php">PHP信息</a></td>
							<td><a href="$script_name?action=server">网站变量</a></td>
							<td><a href="$script_name?action=set">修改帐号密码</a></td>
							<td><a href="$script_name?action=setroot">设置根目录</a></td>
							<td><a href="$script_name?action=kill">自杀功能</a></td>
							<td><a href="$script_name?action=logout">退出登录</a></td>
						</tr>
					</table>
				</div>
OUTPUT;
...
function explorer($dir){
	...
	$foldersnum=0;
	$filesnum=0;
	$action=array('php'=>'编辑','html'=>'编辑','htm'=>'编辑','js'=>'编辑','bak'=>'编辑','txt'=>'编辑','sql'=>'编辑','zip'=>'查看');
	$buttons=array('创建','剪切','复制','粘贴','重命名','删除','修改权限','压缩','解压','FTP上传','下载');
	...
	$table_heder='<table id="showdir"><thead><tr style="background-color:#ccccff;" align="left"><th onclick="SelectAll();" style="cursor:pointer">全选</th><th onclick="sortTable(\'showdir\',1);" style="cursor:pointer">文件名</th><th onclick="sortTable(\'showdir\',2);" style="cursor:pointer">类型</th><th onclick="sortTable(\'showdir\',3,\'int\');" style="cursor:pointer">大小</th><th onclick="sortTable(\'showdir\',4);" style="cursor:pointer">创建时间</th><th onclick="sortTable(\'showdir\',5);" style="cursor:pointer">修改时间</th><th onclick="sortTable(\'showdir\',6);" style="cursor:pointer">权限</th><th onclick="sortTable(\'showdir\',7);" style="cursor:pointer">操作</th></tr><thead>';
	...
	echo '<div class="textleft">此目录共有文件夹：'.$foldersnum.'个；文件：'.$filesnum.'个。<br>'.todir($dir).'</div>'.$buttons_code.$table_heder.'<tbody>'.$folders.$files.'</tbody></table></form><p align="left">注：鼠标点击表头“全选”栏，将选中所有未选文件；鼠标点击表头其它列，会对这一列进行排序显示，页面刷新后顺序还原;本系统只能对特定后缀文件进行编辑，如需编辑其它类型文件，可对文件后缀名修改后进行编辑。</p>';
	$dir_ob->close();
}
		...
		foreach($_FILES['uploads']['error'] as $key=>$error){
			if($error==UPLOAD_ERR_OK){
				...
				if(move_uploaded_file($tmp_name,$dir.'/'.$name)){
					echo $name.'-->上传至<a href="'.SCRIPT_NAME.'?dir='.$ndir.'/">.'.$ndir.'</a>成功;<br><br>';
				}else{
					echo $name.'-->复制失败;<br><br>';
				}
			}else{
				echo $_FILES['uploads']['name'][$key].'-->失败:'.$error.'<br><br>';
			}
		}
	}
	echo '<form action="'.SCRIPT_NAME.'?action=upload" method="post" enctype="multipart/form-data">本地上传：<input name="uploads[]" type="file" multiple />'.listdir(LOTUS_ROOT).'<input type="submit" value="上传" /></form><br><div align="center">**支持一次选择多个文件上传，但需要浏览器支持，如IE9及以上浏览器！**</div>';
}

function teldown($url,$ndir){
	$realpath=LOTUS_ROOT.$ndir;
	if(empty($url)){
		echo '<div>** 远程下载是指从远程直接下载文件到当前服务器（不需要下载后再上传）的一种功能。类似于SSH的Wget功能。**</div><br><table><tr><td>远程文件地址</td><td>服务器存放目录</td></tr><tr><td><form action="'.SCRIPT_NAME.'?action=teldown" method="POST"><input name="url" size="80" /></td><td>'.listdir(LOTUS_ROOT).'<input name="submit" value="下载到本地服务器" type="submit" /></form></td></tr><tr><td colspan="2">注：本功能“远程文件地址”支持多种格式，最常见的分别为“http”和“ftp”协议格式。格式示例如下：<br>http://www.hostname.com/files/filename.zip<br>ftp://username:password@ftp.hostname.com:21/public_html/filename.zip</td></tr></table>';
	}else{
		$url=urldecode($url);
		$name=basename($url);
		$ch=curl_init();
		...
		if(file_put_contents($realpath.$name,$res) && !curl_error($ch)){
			echo '远程文件'.$url.'下载至'.$ndir.'成功!';
			explorer($ndir);
		}else{
			echo '远程文件'.$url.'下载至'.$ndir.'失败!';
		}
		curl_close($ch);
	}
}
...
function lmove($files,$dir,$path){
	foreach($files as $file){
		...
		if(rename(LOTUS_ROOT.$file,LOTUS_ROOT.$path.basename($file))){
			echo $file.'移动至'.$path.'-->OK!<br>';
		}else{
			echo $file.'移动至'.$path.'-->False!<br>';
		}
	}
}
...
function lcopy($files,$dir,$path){
	foreach($files as $file){
		...
		if(copy(LOTUS_ROOT.$file,LOTUS_ROOT.$path.basename($file))){
			echo $file.'复制至'.$path.'-->OK!<br>';
		}else{
			echo $file.'复制至'.$path.'-->False!<br>';
		}
	}
}
...
function lrename($dir,$oldnames,$newnames){
	...
	foreach($oldnames as $name){
		...
		if(rename(LOTUS_ROOT.$dir.$oldnames[$i],LOTUS_ROOT.$dir.$newnames[$i])){
			echo $oldnames[$i].'重命名为'.$newnames[$i].'-->OK!<br>';
		}else{
			echo $oldnames[$i].'重命名为'.$newnames[$i].'-->False!<br>';
		}
		$i++;
	}
}
				...
				if($item!='.' && $item!='..'){
					if(is_dir($realpath.$path.'/'.$item)){
						...
						if(unlink($realpath.$path.'/'.$item)){
							echo $path.'/'.$item.'删除-->OK!<br>';
						}else{
							echo $path.'/'.$item.'删除-->False!<br>';
						}
					}
				}
			}
...
function create($dir,$name,$type){
	$realpath=LOTUS_ROOT.$dir;
	if(empty($name)){
		echo '<form method="post" action="'.SCRIPT_NAME.'?action=fileadmin&dir='.$dir.'" onsubmit="return Ask(\'创建\');">要创建的文件或目录名：<input name="name" type="text"/><br><br>创建类型：目录<input type="radio" name="type" value="dir" checked />	文件<input type="radio" name="type" value="file" /><br><br><input name="submit" type="submit" value="创建" /></form>';
	}else{
		if($type=='dir'){
			if(mkdir($realpath.'/'.$name,0777)){
				echo '目录'.$name.'创建-->OK!';
			}else{
				echo '目录'.$name.'创建-->False!';
			}
		}else{
			if(file_put_contents($realpath.'/'.$name,' ',LOCK_EX)){
				echo '文件'.$name.'创建-->OK!';
			}else{
				echo '文件'.$name.'创建-->False!';
			}
		}
		explorer($dir);
	}
...
function lchmod($dir,$names,$chmods){
	...
	foreach($names as $name){
		if(chmod($realpath.$name,octdec($chmods[$i]))){
			echo $name.'修改权限'.$chmods[$i].'-->OK!';
		}else{
			echo $name.'修改权限'.$chmods[$i].'--False!';
		}
		$i++;
	}
}
...
function download($dir,$file){
	...
	if(is_dir($realpath.$file)){
		maintop('文件下载');
		echo '这是一个目录，请<a href="'.SCRIPT_NAME.'?dir='.$dir.'">返回</a>重新选择文件！';
		mainbottom();
	}else{
...
function lzip($dir,$files,$name){
	if(empty($name)){
		$contents='<form method="post" action="'.SCRIPT_NAME.'?action=fileadmin&dir='.$dir.'"><font color="red">注意：输入的压缩包名称若与当前目录现有文件同名，此文件将被覆盖！</font><br><br>压缩包名称：<input type="text" name="name" />.zip';
		foreach($files as $file){
			$file=urldecode($file);
			$contents.='<input type="hidden" name="file[]" value="'.$file.'" />';
		}
		echo $contents.'<input type="submit" name="submit" value="压缩" /></form>';
	}else{
		$realpath=LOTUS_ROOT.$dir;
		if(class_exists('Zipper')){
			...
			if($res===true){
				...
				$zip->close();
				echo '压缩包'.$name.'.zip已压缩完成by Ziparchive！';
				explorer($dir);
				return true;
			}else{
				echo '压缩失败！请重试！';
...
function lunzip($dir,$zipfile,$folder){
	...
	if($folder===NULL){
		echo '<form method="post" action="'.SCRIPT_NAME.'?action=fileadmin&dir='.$dir.'"><font color="red">注意：执行此操作，压缩包中的文件将有可能覆盖解压目录中的同名文件！</font><br><br>解压至：'.$dir.'<input type="text" name="folder" /><input type="hidden" name="file[]" value="'.$zipfile.'" /><input type="submit" name="submit" value="解压" /></form>';
	}else{
		$realpath=LOTUS_ROOT.$dir;
		if(class_exists('ZipArchive')){
			$zip=new ZipArchive();
...
function setroot($root){
	if(empty($root)){
		echo '**你可以把本程序放置在网站的任何目录，然后通过本功能设置你要的根目录后，就可以对全站文件进行管理了！**<br><br><b>您的当前根目录为：'.LOTUS_ROOT.'</b><br><br><form method="post" action="'.SCRIPT_NAME.'?action=setroot"><input type="text" name="root" value="'.CURRENT_DIR.'" size="50" /><input type="submit" name="submit" value="设置"></form>';
	}else{
...
function set($user,$pass){
	if(empty($user) || empty($pass)){
		echo '**建议设置较复杂的帐号和密码。一般包括大写字母，小写字母，数字，符号等的组合！**<br><br><form method="post" action="'.SCRIPT_NAME.'?action=set"><table align="center"><tr><td>新帐号：</td><td><input type="user" name="user"><br></td></tr><tr><td>新密码：</td><td><input type="password" name="pass"><br></td></tr><tr><td></td><td><input type="submit" name="submit" value="设置"></td></tr></table></form>';
	}else{
		$contents=file_get_contents(CURRENT_DIR.'/'.basename(SCRIPT_NAME));
		...
		if(file_put_contents(CURRENT_DIR.'/'.basename(SCRIPT_NAME),$contents,LOCK_EX)){
			echo '帐号密码成功!请<a href="'.SCRIPT_NAME.'">重新登录</a>';
		}
	}
}
...
function sqlbackup($sqlcon,$tables){
	if(empty($sqlcon)){
		echo '<form action="'.SCRIPT_NAME.'?action=backup" method="post"><table align="center"><tr><td>主机：</td><td><input type="text" name="host" value="localhost" /></td></tr><tr><td>端口：</td><td><input type="text" name="port" value="3306" /></td></tr><tr><td>用户名：</td><td><input type="text" name="user" value="root" /></td></tr><tr><td>密码：</td><td><input type="text" name="password" value="" /></td></tr><tr><td>数据库名：</td><td><input type="text" name="database" value="test" /></td></tr><tr><td>数据表名：</td><td><input type="text" name="tables" value="" /></td></tr><tr><td>字符编码：</td><td><input type="text" name="charset" value="utf-8" /></td></tr><tr><td>保存为：</td><td><input type="text" name="target" value="sql.sql" /></td></tr><tr><td></td><td><input type="submit" value="备份" /></td></tr></table></form>';
	}else{
		$mysql_ob=new DatabaseTool($sqlcon);
		$mysql_ob->backup($tables) or die('备份发生错误：'.$mysql_ob->getError());
	}
}

function sqlstore($sqlcon,$path){
	if(empty($sqlcon)){
		echo '<form action="'.SCRIPT_NAME.'?action=store" method="post"><table align="center"><tr><td>主机：</td><td><input type="text" name="host" value="localhost" /></td></tr><tr><td>端口：</td><td><input type="text" name="port" value="3306" /></td></tr><tr><td>用户名：</td><td><input type="text" name="user" value="root" /></td></tr><tr><td>密码：</td><td><input type="text" name="password" value="" /></td></tr><tr><td>数据库名：</td><td><input type="text" name="database" value="test" /></td></tr><tr><td>字符编码：</td><td><input type="text" name="charset" value="utf-8" /></td></tr><tr><td>源文件：</td><td><input type="text" name="target" value="sql.sql" /></td></tr><tr><td></td><td><input type="submit" value="恢复" /></td></tr></table></form>';
	}else{
		if(empty($path))die('请输入源数据文件路径!');
		$mysql_ob=new DatabaseTool($sqlcon);
		$mysql_ob->restore($path) or die('恢复发生错误：'.$mysql_ob->getError());
	}
}

function fileadmin($dir){
	$note='您没有选择任何文件！请<a href="'.SCRIPT_NAME.'?dir='.$dir.'">返回</a>重选。';
	switch($_POST['submit']){
		case '创建':
		...
		if(file_exists(LOTUS_TMP) || mkdir(LOTUS_TMP,0755,true)){
			if(file_put_contents(LOTUS_TMP.CLIP,serialize($filelist),LOCK_EX)){
				echo '<script language="JavaScript">alert("你选择的文已剪切到剪贴板，请打开或选择目标文件夹，点“粘贴”移动文件！");</script>';
				explorer($dir);
			}
		}
		break;
		...
		if(file_exists(LOTUS_TMP) || mkdir(LOTUS_TMP,0755,true)){
			if(file_put_contents(LOTUS_TMP.CLIP,serialize($filelist),LOCK_EX)){
				echo '<script type="text/JavaScript">alert("你选择的文已复制到剪贴板，请打开或选择目标文件夹，点“粘贴”移动文件！");</script>';
				explorer($dir);
			}
		}
		break;
		...
		foreach($_POST['file'] as $file){
			$file=urldecode($file);
			$contents.='<tr><td><input type="hidden" name="oldname[]" size="20" value="'.$file.'" />'.$file.'重命名为：</td><td><input name="newname[]" size="20" /></td></tr>';
		}
		echo '<form action="'.SCRIPT_NAME.'?action=rename" method="post"><table align="center">'.$contents.'</table><input type="hidden" name="dir" value="'.$dir.'" /><input type="submit" value="提交" /></form><br>';
		break;
		case '删除':
		if(empty($_POST['file'])){
			echo $note;
		...
		foreach($_POST['file'] as $file){
			$file=urldecode($file);
			$contents.='<tr><td><input type="hidden" name="name[]" size="20" value="'.$file.'" />'.$file.'权限修改为：</td><td><input name="chmod[]" size="10" /></td></tr>';
		}
		echo '<form action="'.SCRIPT_NAME.'?action=chmod&dir='.$dir.'" method="post"><table align="center">'.$contents.'</table><input type="submit" value="提交" /></form><br>';
		break;
		case '压缩':
		if(empty($_POST['file'])){
			echo $note;
...
function ftp_creat_task($pathlists=array()){
	...
	echo '创建上传任务列表败，请检查脚本目录是否可写。';
	return false;
}

function ftp_weblogin($action){
		echo '<font class=error>**登录后你可以对远程FTP服务器上的文件进行操作!**</font><br><br><form action="'.SCRIPT_NAME.'?action='.$action.'" method="POST">FTP 地址:  <input name="host" size="30" /><br>FTP 端口:  <input name="port" size="30" value="21" /><br>FTP 用户:  <input name="user" size="30" /><br>FTP 密码:  <input name="pass" size="30" /><br>远程目录:  <input name="ftpdir" size="30" value="/" /><br><br><br><input type="submit" value="登录" /></form>';
}

function ftp_access($host,$port,$user,$pass){
	if(!$conn_id=ftp_connect($_SESSION['host'],$_SESSION['port'])){
		echo 'ftp服务器'.$_SESSION['host'].'连接失败！';
		return false;
	}
	if(ftp_login($conn_id,$_SESSION['user'],$_SESSION['pass'])){
		return $conn_id;
	}else{
		echo 'ftp服务器'.$_SESSION['host'].'登录失败！';
		return false;
	}
}
...
function ftp_explorer($conn_id,$remotedir){
	...
	$contents3='';
	$buttons_code='';
	$buttons=array('上传至此目录','创建文件夹','选择要移动文件','移动到此目录','重命名','删除','修改权限');
	$s='';
	$tmp=array();
	$file=array();
	$files=array();
	if($conn_id){
		...
		if(!@ftp_chdir($conn_id,$remotedir)){
			echo '打开远程FTP服务器目录'.$remotedir.'失败，请重试！';
			return false;
		}
		...
		$contents1='<div class="textleft">此目录共有文件夹：'.$dirnum.'个；文件：'.$filenum.'个。<br>'.todir($remotedir,'ftp').'</div><form  name="myform" method="post" action="'.SCRIPT_NAME.'?action=ftpadmin&ftpdir='.$remotedir.'"><div align="right">'.$buttons_code.'</div><table width=100%  border="1" cellpadding="2" cellspacing="0"><tr align="left"><th></th><th>文件名</th><th>类别</th><th>修改时间</th><th>大小</th><th>权限</th><th>uid</th><th>gid</th></tr>';
...
function ftp_admin($ftpdir){
	$filelist=array();
	$note='您没有选择任何文件，返回至<a href="'.SCRIPT_NAME.'?action=ftp&ftpdir='.$ftpdir.'">'.$ftpdir.'</a>选择。';
	switch($_POST['submit']){
		case '上传至此目录':
		...
		if(empty($tasklist)){
			...
			if($conn_id=ftp_access($_SESSION['host'],$_SESSION['port'],$_SESSION['user'],$_SESSION['pass'])){
				...
				echo '返回至<a href="'.SCRIPT_NAME.'?action=ftp&ftpdir='.$ftpdir.'">'.$ftpdir.'</a>查看。';
			}
		}
		break;
		case '创建文件夹':
			echo '文件夹名称：<form action="'.SCRIPT_NAME.'?action=ftpmkdir" method="post"><input name="ftpmkdir" size="20" /><input type="hidden" name="ftpdir" value="'.$ftpdir.'" /><input type="submit" value="创建" /></form><br>**请输入要创建的文件夹名称!**<br>';
		break;
		case '选择要移动文件':
		...
		if(file_exists(LOTUS_TMP) || mkdir(LOTUS_TMP,0755,true)){
			if(file_put_contents(LOTUS_TMP.FTP_CLIP,serialize($filelist),LOCK_EX)){
				echo '<script language="JavaScript">alert("你选择的文已剪切到剪贴板，请进一步选择目标文件夹，点“移动到此目录”移动文件！");</script>';
				if($conn_id=ftp_access($_SESSION['host'],$_SESSION['port'],$_SESSION['user'],$_SESSION['pass'])){
		...
		echo '<form action="'.SCRIPT_NAME.'?action=ftprename" method="post"><table align="center">'.$contents.'</table><input type="hidden" name="ftpdir" value="'.$ftpdir.'" /><input type="submit" value="提交" /></form><br>';
		break;
		case '删除':
		if(!isset($_POST['ftpfile'])){
			echo $note;
		...
		$contents='';
		foreach($_POST['ftpfile'] as $file){
			$contents.='<tr><td>'.$file.'权限修改为：</td><td><input type="hidden" name="ftpfile[]" value="'.$file.'" /><input name="chmod[]" size="20" /></td></tr>';
		}
		echo '<form action="'.SCRIPT_NAME.'?action=ftpchmod&" method="post"><table align="center">'.$contents.'</table><input type="hidden" name="ftpdir" value="'.$ftpdir.'" /><input type="submit" value="提交" /></form><br>';
		break;
		default:
...
function ftp_upload($conn_id,$remotedir,$tasklist,$mode=FTP_BINARY){
	...
	ftp_chdir($conn_id,$remotedir) or die('远程目录'.$remotedir.'切入错误！');
	foreach($tasklist as $transing){
		if(is_dir($transing)){
			if(@ftp_mkdir($conn_id,$transing)){
				echo $transing.'在远程'.$remotedir.'创建-->OK!<br>';
			}else{
				echo $transing.'在远程'.$remotedir.'创建-->False!<br>';
			}
		}else{
			if(ftp_put($conn_id,$transing,$transing,$mode)){
				echo $transing.'上传至远程'.$remotedir.'-->OK!<br>';
			}else{
				echo $transing.'上传至远程'.$remotedir.'-->False!<br>';
			}
		}
	}
	ftp_close($conn_id);
...
function ftpchmod($conn_id,$ftpdir,$modes,$files){
	...
	foreach($files as $file){
		if(ftp_chmod($conn_id,octdec($modes[$i]),$ftpdir.$file)){
			echo $file.'的权限已修改为：'.$modes[$i].'！';
		}else{
			echo $file.'权限修改-->False!';
		}
		$i++;
	}
}
...
function edit($dir){
	if(isset($_POST['file']) && isset($_POST['contents'])){
		...
		if(file_put_contents($name,$contents,LOCK_EX)){
			echo '文件'.$file.'保存成功！'."\n".'<a href="'.SCRIPT_NAME.'?dir=/">返回文件目录</a>';
		}else{
			echo '文件保存出错！'."\n".'<a href="'.SCRIPT_NAME.'?dir=/">返回文件目录</a>';
		}
	}elseif(isset($_GET['file'])){
		$edit=array('php'=>'编辑','htm'=>'编辑','tml'=>'编辑','.js'=>'编辑','bak'=>'编辑','txt'=>'编辑','sql'=>'编辑');
...
function zipview($dir,$file){
	...
	$zip=new PclZip($realpath.$file);
		...
		echo '<div align="center">压缩容器<b>'.substr($file,1).'</b>中包含文件列表</div><table width=100%  border="1" cellpadding="2" cellspacing="0"><tr><th>序号</th><th>文件名</th><th>存贮名</th><th>文件大小</th><th>压缩后大小</th><th>修改时间</th><th>注释</th><th>是否目录</th><th>状态</th><th>CRC校验值</th></tr>'.$contents.'</table>';
}

function killself($sure){
	if(empty($sure)){
		echo '<form action="'.SCRIPT_NAME.'?action=kill" method="post"><input type="hidden" name="sure" value="killself" /><font color="red"><b>注意：执行自杀操作后，程序将自身文件从服务器永久删除！本程序将不可再用！</b></font><br><br><input type="submit" name="submit" value="确定自杀" />		<input type="submit" name="submit" value="取消自杀" /></form>';
	}elseif($sure=='killself' && $_POST['submit']=='确定自杀'){
		if(unlink(CURRENT_DIR.SCRIPT_NAME)){
			echo '自杀成功!';
		}else{
			echo '自杀失败！请重试！';
		}
	}elseif($_POST['submit']=='取消自杀'){
		echo '自杀操作已取消！';
	}
}
...
switch($action){
	...
	$name=isset($_POST['name'])?$_POST['name']:NULL;
	if($name===NULL){
		echo '<form method="post" action="'.SCRIPT_NAME.'?action=sitezip"><font color="red">注意：输入的压缩包名称若与根目录现有文件同名，此文件将被覆盖！</font><br><br>压缩包名称：<input type="text" name="name" value="allbackup"/>.zip<input type="submit" name="submit" value="全站压缩备份" /></form>';
	}else{
		$dir='/';
